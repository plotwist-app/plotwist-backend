...
import { ssoProvidersInAuth, samlProvidersInAuth, flowStateInAuth, samlRelayStatesInAuth, usersInAuth, identitiesInAuth, sessionsInAuth, refreshTokensInAuth, mfaFactorsInAuth, oneTimeTokensInAuth, mfaAmrClaimsInAuth, mfaChallengesInAuth, ssoDomainsInAuth, profiles, followers, lists, listLikes, recommendations, reviews, watchedListItems, likes, reviewReplies, subscriptions, watchListItems, listItems } from "./schema";
...
export const watchedListItemsRelations = relations(watchedListItems, ({one}) => ({
	profile: one(profiles, {
...
export const reviewRepliesRelations = relations(reviewReplies, ({one, many}) => ({
	likes: many(likes),
	review: one(reviews, {
